...
// The second row lists the physical input and output channels on Arduino for B,W,P, and the SPI CS pin is listed for S. Use 0 for UART and USB.
  ...
  #if ETHERNET_COM == 0
    byte InputHW[] = {'U','U','U','U','U','X','P','P','P','P','P','P'};
    byte InputCh[] = {0,0,0,0,0,0,39,38,18,15,5,6};                                         
    byte OutputHW[] = {'U','U','U','U','U','X','P','P','P','P','P','P','V','V','V','V','V','V'};
    byte OutputCh[] = {0,0,0,0,0,0,37,14,16,17,29,30,23,22,20,21,3,4};  
  #else
...
byte FullStatePath[65000] = {0}; // A list of all the states that were passed through on this run of the state matrix
byte NewStateCounter = 0; // used to count how many states we've added to FullStatePath
byte NewStateCounterRollover = 0; // used to track NewStateCounter values above 256
uint16_t NewStateCounterFullBit = 0; // a 16 bit counter that keeps track of the position to index in FullStatePath 
...
uint32_t GlobalTimersTrigTransmit = {0}; // used to transmit the state of triggered timers back to matlab
boolean newTimerTriggered = false; // true if any new timers were triggered on this hardware cycle
boolean ForceStatusEvent  = false; // if true forces a status event to be sent
boolean DoStateChangeTransmission = false; // if true a transmission is sent identifying the new state
...
uint32_t CyclesSinceLastEvent = 0; //counter to send a status event on
...
void handler() { // This is the timer handler function, which is called every (timerPeriod) us
  ...
  if ((PC.available() > 0) && (smaPending == false)) { // If a message has arrived on the USB serial port
    ...
    switch (CommandByte) {
      ...
      break;
      case 'N':  // Force a new state
        Byte1 = PC.readByte();
        previousState = uint8_t(CurrentState);
        CurrentState  = (int)Byte1;
        if (CurrentState > nStates) {
          CurrentState = nStates;
        }
        NewState = CurrentState;
        FullStatePath[NewStateCounterFullBit] = NewState;
        NewStateCounterFullBit++;
        NewStateCounter++;
        if (NewStateCounter == 0) {
          NewStateCounterRollover++;
        }
        ForceStatusEvent = true;
        StateStartTime = CurrentTime;
        setStateOutputs(CurrentState);
      break;
      case 'A': // Return time info
        PC.writeUint32(CurrentTime);
        PC.writeUint32(currentTimeMicros);
        PC.writeUint32(nMicrosRollovers);
      break;
      case 'Y': // Return time info
        PC.writeUint32(MatrixStartTimeMicros);
        PC.writeUint32(MatrixEndTimeMicros);
        PC.writeUint32(nCyclesCompleted); 
      break;
      case '@': // Return FullStatePath
        FullStatePath[1] = NewStateCounter;
        FullStatePath[2] = NewStateCounterRollover;
        PC.writeByteArray(FullStatePath, NewStateCounterFullBit);
      break;
    ...
    if (firstLoop == 1) { 
      ...
      while ((!StateTransitionFound) && (i < nCurrentEvents)) {
        ...
        if (NewState != CurrentState) {
          ...
          DoStateChangeTransmission = true;
          if (NewStateCounterFullBit < 65000) {
            FullStatePath[NewStateCounterFullBit] = NewState;
            NewStateCounterFullBit++;
            NewStateCounter++;
            if (NewStateCounter == 0) {
      ...
      // every 10s or everytime a wave is triggered send a status event
      if ((CyclesSinceLastEvent >= 100000) || (newTimerTriggered) || (ForceStatusEvent)) {
        serialByteBuffer[0] = 3; // Op Code for sending events
        serialByteBuffer[1] = 18; // read 18 events 
        serialByteBuffer[2] = (uint8_t)previousState;
        serialByteBuffer[3] = (uint8_t)CurrentState;  
        PC.writeByteArray(serialByteBuffer, 4);
        
        timeBuffer.uint32[0] = CurrentTime;
        for (int i = 0; i < 20; ++i) {
          bitWrite(GlobalTimersTrigTransmit,i,GlobalTimersTriggered[i]);
        }
        if (newTimerTriggered) {
          bitWrite(GlobalTimersTrigTransmit,31,true);
        } else {
          bitWrite(GlobalTimersTrigTransmit,31,false);
        }
        if (ForceStatusEvent) {
          bitWrite(GlobalTimersTrigTransmit,30,true);
        } else {
          bitWrite(GlobalTimersTrigTransmit,30,false);
        }
        timeBuffer.uint32[1] = (uint32_t)GlobalTimersTrigTransmit;
        timeBuffer.uint64[1] = sessionTimeMicros();
        PC.writeByteArray(timeBuffer.byteArray, 16);

        newTimerTriggered = false;
        ForceStatusEvent  = false;
      ...
      //if (DoStateChangeTransmission) {
      //    serialByteBuffer[0] = 2; // Op Code for sending states
      //    serialByteBuffer[1] = 1; // read 1 byte 
      //    serialByteBuffer[2] = (uint8_t)NewState; 
      //    PC.writeByteArray(serialByteBuffer, 3);
      //   DoStateChangeTransmission = false;
      //}
  ...
  if (MatrixFinished) {
    ...
    //transmit the full state path, this is a backup to confirm nothing was missed in the piecewise realtime transmissions
    FullStatePath[1] = NewStateCounter;
    FullStatePath[2] = NewStateCounterRollover;
    PC.writeByteArray(FullStatePath, NewStateCounterFullBit);
...
void startSM() {  
  ...
  MatrixFinished = false;
  FullStatePath[0] = 4; //special op code
  FullStatePath[1] = 1; //placeholder for NewStateCounter value
  FullStatePath[2] = 0; //placeholder for NewStateCounterRollover value
  FullStatePath[3] = 0; //we start in state 0
  NewStateCounter         = 1;
  NewStateCounterRollover = 0;
  NewStateCounterFullBit  = 4; // next position to index in FullStatePath
  #if LIVE_TIMESTAMPS == 0
    if (usesFRAM) {
      // Initialize fRAM
